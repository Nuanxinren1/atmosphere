    ...
    declare, lang, _WidgetBase, domConstruct, dom, query, domStyle, on, DynamicMapServiceLayer, screenUtils, Point) {
        return declare("CustomDynamaticLayer", DynamicMapServiceLayer, {
            ...
            constructor: function (map,srcCanvas,config, args) {
                ...
                this.colors=[[255,255,255,180],[0,0,254,255],[0,75,255,255],[0,255,255,255],[129,181,202,255],
                                [5,109,152,255],[0,212,42,255],[98,255,0,255],[255,252,0,255],[254,0,0,255]
                            ];
            ...
            createImage:function (mapextent,delta) {
                ...
                for (let i = 0; i < this.config.width; i++) {
                    ...
                    for (let j = 0; j < this.config.height; j++) {
                        ...
                        values.push(parseFloat(srcData[index][2]));
                    }
                    newsrcdata[i]=values;
                }
                ...
                for (let i = 0; i < newwidth; i++) {//根据新的行列求出原图像的行列号
                    var data=[];
                    for (let j = 0; j < newheight; j++) {
                        ...
                        try {
                                //var newvalue=this.interpolate(ddx,ddy,newsrcdata[srcx][srcy],
                               // newsrcdata[srcx+1][srcy],newsrcdata[srcx+1][srcy-1],newsrcdata[srcx][srcy-1]);
                ...
                console.log("tranformMax:"+tranformMax+";tranformMin"+tranformMin);
				...
				for (var i = 0; i < imgData.data.length; i += 4) {
                    ...
                    var level=parseInt((valueAtRowCol-tranformMin)*9/(tranformMax-tranformMin)); 
